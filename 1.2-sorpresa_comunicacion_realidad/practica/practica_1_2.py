...
def p_csr_p(r: int, c: int, s: int, p: float) -> float:
    ...
    return p * p_rcs_M(r,c,s,1) + (1-p) * p_rcs_M(r,c,s,0)
...
def p_p_datos(p: float, datos: List[Tuple[int, int, int]]) -> float:
    ...
    p ∈ [0, 1]
    """
    prod_verosimilitud = 1.0
    for c, s, r in datos:
        prod_verosimilitud *= p_csr_p(r, c, s, p)

    numerador_actual = prod_verosimilitud * p_p(p)

    denominador = 0.0
    for pg in P_GRID:
        prod_v_grilla = 1.0
        for c, s, r in datos:
            prod_v_grilla *= p_csr_p(r, c, s, pg)
        denominador += prod_v_grilla * p_p(pg)


    return numerador_actual / denominador
...
def p_episodio_datos_MA(
    ...
    for p in P_GRID:
        post = p_p_datos(p,datos)
        vero = p_csr_p(r,c,s,p)
        prediccion_total += post * vero

    return prediccion_total
...
def p_datos_MA(datos: List[Tuple[int, int, int]]) -> float:
    ...
    posterior_p = np.array([p_p(p) for p in P_GRID])

    for c,s,r in datos:
        pred_episodo = 0
        for i, p in enumerate(P_GRID):
            pred_episodo += p_csr_p(r,c,s,p) * posterior_p[i]

        prob_total *= pred_episodo

        for i, p in enumerate(P_GRID):
            posterior_p[i] = posterior_p[i] * p_csr_p(r,c,s,p)
...
def log_p_datos_MA(datos: List[Tuple[int, int, int]]) -> float:
    ...
    # Inicializamos el posterior de p de forma uniforme sobre la grilla
    posterior_p = np.array([p_p(p) for p in P_GRID])
    
    for c, s, r in datos:
        # 1. Calculá la predicción del episodio actual usando el posterior acumulado como pesos
        pred_episodio = 0.0
        for i, p in enumerate(P_GRID):
            pred_episodio += p_csr_p(r, c, s, p) * posterior_p[i]
            
        # 2. Sumá el log10 de esa predicción al acumulador total
        log_prob_total += np.log10(pred_episodio)
        
        # 3. Actualizá el posterior de p de forma incremental para el próximo paso (Bayes)
        for i, p in enumerate(P_GRID):
            posterior_p[i] = posterior_p[i] * p_csr_p(r, c, s, p)
        # Normalizá el posterior_p para que sume 1
        posterior_p /= np.sum(posterior_p)
...
def log_media_geometrica(datos: List[Tuple[int, int, int]], m: int) -> float:
    ...
    """
    n = len(datos) * 3
    return (1/n) * log_pDatos_M(datos, m)
...
def log_media_geometrica_MA(datos: List[Tuple[int, int, int]]) -> float:
    ...
    """
    n = len(datos) * 3
    return (1/n) * log_p_datos_MA(datos)
...
def p_datos_con_alternativo(datos: List[Tuple[int, int, int]]) -> float:
    ...
    """
    c0 = 10 ** log_pDatos_M(datos,0)
    c1 = 10 ** log_pDatos_M(datos,1)
    ca = 10 ** log_p_datos_MA(datos)

    return (c0 * pM_con_alternativo(0)) + (c1 * pM_con_alternativo(1)) + (ca * pM_con_alternativo("MA"))
...
def pM_datos_con_alternativo(
    ...
    m ∈ {0, 1, 'MA'}
    """
    prior = pM_con_alternativo(m)

    match m:
        case 0:
            verosimilitud = 10** log_pDatos_M(datos, 0)
        case 1:
            verosimilitud = 10** log_pDatos_M(datos, 1)
        case "MA":
            verosimilitud = 10** log_p_datos_MA(datos)

    denominador = p_datos_con_alternativo(datos)
    if denominador == 0:
        return 0.0
    return (verosimilitud * prior) / denominador
...
def evolucion_posterior_con_alternativo(
    ...
    for t in range(len(datos)):
        dat = datos[:t+1]
        post_t = pM_datos_con_alternativo(m,dat)
        resultado.append(post_t)
